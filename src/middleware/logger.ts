    ...
    transports: [
        ...
        new transports.Console({ 
            ...
            format: format.combine(
                ...
                format.printf(info => `${colors.green(`[${info.timestamp}]`)} ${info.message}`)
            )
        })
    ]
});
...
export default (req: Request, res: Response, next: NextFunction) => {
    ...
    res.on('finish', () => {
        // Log the request
        logger.info(`${req.ip} - ${colors.yellow(req.method)} ${req.baseUrl}${req.path} ${res.statusCode} - ${colors.blue(`${Date.now() - start}ms`)}`);
    });
